...
if (input$test_type == 'TG201'){
  ...
  SD_data <- filedata() %>%
    mutate( gr0to24 = log(H24 / H0),
            ...
            gr24to72 = log(H72 / H24)/(2) ) %>%
   group_by(CONC) %>% 
   summarize_at(vars(-"DATAID"),.funs= c(sd)) 
  list( "Mean" = knitr::kable( Mean_data ),  "SD"=  knitr::kable( SD_data ) )
